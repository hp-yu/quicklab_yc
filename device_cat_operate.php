...
<?php
 if ($_REQUEST['type']=='export_excel')
 {
 	...
 	export_excel('device_cat',$query);
 	exit;
 }
?>
<?php
  do_html_header_begin('Device categories operate-Quicklab');
?>
...
$(document).ready(function() {
  ...
  <table width="100%" class="operate" >
	<tr><td colspan='2'><div align='center'><h2>Device categories</h2></div></td></tr>
      <tr>
        <td colspan="2"><h3>Add a device category: </h3></td>
        </tr>
      <tr>
        <td width="20%">Name:</td>
...
function editform()
{
  ...
  $device_cat = get_record_from_id('device_cat',$_REQUEST['id']);
  ?>
    <script type="text/javascript">
$(document).ready(function() {
	$("#edit_form").validate({
	...
	<tr><td colspan='2'><div align='center'><h2>Device categories</h2></div></td></tr>
      <tr>
        <td colspan="2"><h3>Edit device category:</h3></td>
        </tr>
      <tr>
        <td width="20%">Name: </td>
        <td width="80%"><input type='text' name='name' id="name" size="40" value="<?php
  echo stripslashes(htmlspecialchars($device_cat['name']));?>">*</td>
      </tr>
      <tr>
        <td colspan="2"><input type="submit" name="Submit" value="Submit" />
        <a href='<?php echo $_SESSION['url_1'];?>'>
...
function add()
{
  ...
  try {
	...
	header('Location: device_cat.php?id='.$id);
    }
    catch (Exception $e)
    {
