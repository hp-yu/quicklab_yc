...
?>
<?php selectedRequest('supplies');?>
<?php
  do_html_header('Supplies-Quicklab');
  do_header();
  do_leftnav();
?>
<?php
...
<form action="" method='get' name='search' target='_self'>
  <table width='100%' class='search'>
	...
	search_header('Supplies');
	search_keywords('supplies',$fields);
	?>
  <tr>
    <td colspan='2' height='21' valign='top'>And supply category:<?php
      $query= "select * from supply_cat ORDER BY name";
      echo query_select_all('supply_cat_id', $query,'id','name',$_REQUEST['supply_cat_id']);?>
    </td>
  </tr>
	<?php
	search_project();
...
<?php
  ...
  if($_REQUEST['supply_cat_id']==null){$_REQUEST['supply_cat_id']='%';}//special
  ...
  $query =  "SELECT *,
  	...
  	AND mask LIKE '$mask'
  	AND supply_cat_id LIKE '$supply_cat_id'
  	$mask_str ORDER BY $sort $order";
  $_SESSION['query']=$query;//used for EXCEL export

  pagerForm('supplies',$query);
  searchResultsForm('supplies',$results);
?>
</td>
<?php
do_rightbar();
