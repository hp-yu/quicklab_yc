...
?>
<?php
  do_html_header('Supplies operate-Quicklab');
  do_header();
  do_leftnav();
  standard_form();
  do_rightbar();
...
function add_form()
{
	...
	<tr><td colspan='2'><h3>Add new supply:</h3></td>
      </tr>
      <tr>
        <td width='20%'>Name:</td>
      ...
      <tr>
        <td>Supply category:</td><td><?php
		$query= "SELECT * FROM supply_cat ORDER BY name";
		echo query_select_choose('supply_cat_id', $query,'id','name',$_POST['supply_cat_id']);?>
        </td>
      </tr>
      <tr>
        <td>Note:</td>
...
function edit_form()
{
  ...
  if(!userPermission('2',$supply['created_by']))
  {
  	alert();
  }
  ?>
  ...
  echo stripslashes(htmlspecialchars($supply['name']));?>">*</td>
      </tr>
      <tr>
        <td>Project:</td><td><?php
     ...
     echo $supply['description'];?></textarea></td>
      </tr>
      <tr>
      <tr>
        <td>Supply category:</td><td><?php
		$query= "SELECT * FROM supply_cat";
		echo query_select_choose('supply_cat_id', $query,'id','name',$supply['supply_cat_id']);?>
        </td>
      </tr>
      <tr>
        <td>Note:</td>
        <td><textarea name='note' cols='60' rows='4'><?php
     echo $supply['note'];?></textarea></td>
      </tr>
      <tr>
        <td>Mask:</td>
        <td><?php
        $mask=array(array(1,'yes'),
        			array(0,'no'));
        echo option_select('mask',$mask,2,$supply['mask']);
        ?>
        </td>
      </tr>
      <tr>
...
?>
  <form name="relation" method="post" action="" target="_self">
    <tr><td colspan="2"><h3>Relate supply: <?php echo $supply['name']; ?> to these items below:</h3></td></tr>
    <tr><td width="200">
  	...
  	<?php
      $db_conn = db_connect();
      $module=get_record_from_name('modules','supplies');
...
?>
      ...
      <a href="supplies_operate.php?type=edit&id=<?php echo $supply['id']?>"/><img src="./assets/image/general/edit.gif" border="0"/></a></h3></td>
      </tr>
      <tr>
        <td width='20%'>Name:</td>
        <td width='80%'><?php echo $supply['name'];?></td>
      </tr>
      <tr>
        <td>Project:</td><td><?php
		$project= get_record_from_id('projects',$supply['project']);
		echo $project['name'];?></td>
		  </tr>
      <tr>
        <td>Description:</td>
        <td><?php echo wordwrap($supply['description'],70,"<br/>");?></td>
      </tr>
      <tr>
        <td>Supply category:</td><td><?php
		$project= get_record_from_id('supply_cat',$supply['supply_cat_id']);
		echo $project['name'];?></td>
		  </tr>
      <tr>
        <td>Note:</td>
        <td><?php echo wordwrap($supply['note'],70,"<br/>");?></td>
      </tr>
      <tr>
        <td>Created by:</td><td><?php
		$people = get_name_from_id('people',$supply['created_by']);
		echo $people['name'].'  '.$supply['date_create'];?></td></tr>
     </tr>
      <tr>
        <td>Updated by:</td><td><?php
		$people = get_name_from_id('people',$supply['updated_by']);
		echo $people['name'].'  '.$supply['date_update'];?></td></tr>
	 <?php
       $db_conn = db_connect();
       $module=get_record_from_name('modules','supplies');
	   $query="SELECT * FROM items_relation
...
function delete_form()
{
  ...
  {
	$module = get_id_from_name('modules','supplies');
	$db_conn=db_connect();
	$query = "SELECT *
    ...
    echo "<tr><td colspan='2'><h3>Are you sure to delete the supply: ";
    echo $supply['name'];
	echo "?</h3></td>
      </tr>
      <tr>
	...
	{
		echo "<tr><td><h3>This supply related to ";
  ...
  elseif($_SESSION['selecteditemDel'])//multiple delete
  {
    ...
    echo "<tr><td colspan='2'><h3>Are you sure to delete the $num_selecteditemDel supply(s)?<br>
    supply related to other items, storages, orders <br>can not be deleted.</h3></td></tr> ";
...
<tr><td colspan="2">*For the 'supply_cat_id', use supply caterogy id (find it in the supply caterogies module), default 'others'.</td></tr>
...
<tr><td colspan="2">*For the 'mask', use 0 represents no and 1 represents yes, default 0.</td></tr>
  ...
  try {
  	...
  	if((isset($_FILES['file']['name']) && is_uploaded_file($_FILES['file']['tmp_name']))) {
  		$db_conn=db_connect();
  		$query="DROP TABLE IF EXISTS temp_supplies";
  		$result=$db_conn->query($query);

  		$query="CREATE TEMPORARY TABLE IF NOT EXISTS temp_supplies SELECT * FROM supplies WHERE 0";
  		$result=$db_conn->query($query);

  		$query="ALTER TABLE temp_supplies MODIFY id INT(9) UNSIGNED  NOT NULL AUTO_INCREMENT PRIMARY KEY";
  		$result=$db_conn->query($query);
  		...
  		while ($data=fgetcsv($fp)) {
  			if ($n>0) {
  				...
  				$supply_cat_id=mb_convert_encoding(addslashes(trim($data[2])),"utf-8","gb2312");
  				$query="SELECT * FROM supply_cat WHERE id='$supply_cat_id'";
  				$result=$db_conn->query($query);
  				if($result->num_rows==0) $supply_cat_id=1;
  				...
  				$query="INSERT INTO temp_supplies
       	 	(name,description,supply_cat_id,date_create,created_by,project,note,mask)
       		VALUES
      		('$name','$description','$supply_cat_id','$date_create','$created_by','$project','$note','$mask')";
  				$result=$db_conn->query($query);
  				if(!$result) {
  		...
  		$rand=mt_rand();
  		$filename = "temp/supplies_import_$rand.txt";
  		move_uploaded_file($_FILES['file']['tmp_name'],'./'.$filename);

  		$query="SELECT * FROM temp_supplies";
  		$results=$db_conn->query($query);
...
supply_cat_id*</td><td class='results_header'>
...
mask*</td></tr>
	    ...
	    while ($matches = $results->fetch_array()) {
	    	...
	    	$query="SELECT * FROM supply_cat WHERE id='{$matches['supply_cat_id']}'";
	    	$result=$db_conn->query($query);
	    	$supply_cat=$result->fetch_assoc();
	    	echo "<td class='results'>".$supply_cat['name']."</td>";
...
function add()
{
  ...
  if (!filled_out(array($_REQUEST['name'],$_REQUEST['project'],$_REQUEST['created_by'])))
  {
    ...
    $description = $_REQUEST['description'];
		$supply_cat_id = $_REQUEST['supply_cat_id'];
	...
	header('Location: supplies.php?id='.$id);
  }
  catch (Exception $e)
  {
  	echo '<table class="alert"><tr><td><h3>'.$e->getMessage().'</h3></td></tr></table>';
...
function delete()
{
  ...
  if(isset($_REQUEST['id'])&&$_REQUEST['id']!='')//single delete
  {
    $query = "delete from supplies where id = '{$_REQUEST['id']}'";
    $result = $db_conn->query($query);
  }
  elseif($_SESSION['selecteditemDel'])//multiple delete
  {
    ...
    for($i=0;$i<$num_selecteditemDel;$i++)
    {
	  ...
	  $query = "SELECT id
  	  ...
  	  if($relateditem_count==0&&$storage_count==0&&$order_count==0)
  	  {
  	  	$query = "delete from supplies where id = '{$selecteditemDel[$i]}'";
    	$result = $db_conn->query($query);
  	  }
    }
  }
...
function export_excel($module_name,$query)
{
  ...
  {
    $query="SELECT name from supply_cat WHERE id='{$row['supply_cat_id']}'";
		$result = $db_conn->query($query);
		$supply_cat=$result->fetch_assoc();
